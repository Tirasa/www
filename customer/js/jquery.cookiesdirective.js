...
;(function($) {
	...
	var disclosure = function(options) {
		...
		html += '<div style="position:relative;height:auto;width:65%;padding:10px;margin-left:auto;margin-right:auto;text-align:justify;">';
		...
		if(settings.explicitConsent) {
			...
			html += 'font-size:' + settings.fontSize + ';" href="'+ settings.privacyPolicyUri + '">'+ settings.privacyPolicyLinkText +'</a>.';
			...
			html += '<div id="epdnotick" style="display:none;" class="alert-msg"><span>' + settings.explicitCheckboxLabel + '</span></div>'
			...
			html += '<div style="text-align: right;float:right;"><input type="submit" name="explicitsubmit" id="explicitsubmit" value="' + settings.explicitCookieAcceptButtonText + '" class="rnd5 button violet gradient small"/></div><br/></div></div>';
			...
			html += 'font-weight:bold;font-size:' + settings.fontSize + ';" href="'+ settings.privacyPolicyUri + '">' + settings.privacyPolicyLinkText + '</a>.';
			html += '<div style="margin-top:5px;';
			if(settings.inlineAction) {
				html += 'display:inline-block;margin-left:5px';
			}
